function count(word) {
    var letters = {};
    for (var _i = 0, word_1 = word; _i < word_1.length; _i++) {
        var chr = word_1[_i];
        letters[chr] = letters.hasOwnProperty(chr) ? letters[chr] + 1 : 1;
    }
    return letters;
}
function findStartOfPacketMarker(datastreamBuffer) {
    var lastFourCharacters = "";
    console.log("datastreamBuffer", datastreamBuffer);
    for (var i = 0; i < datastreamBuffer.length; i++) {
        // Create 4 letter string and move up the string each iteration
        lastFourCharacters = datastreamBuffer.slice(i, i + 4);
        // count number of occurances
        var letterCount = count(lastFourCharacters);
        // If there are mote than 3 letters counted in
        // length 4 string then they must all be distinct
        if (Object.keys(letterCount).length > 3) {
            console.log(count(lastFourCharacters));
            console.log(i + 4);
            console.log("found it sucker");
            return;
        }
    }
    return -1;
}
// findStartOfPacketMarker("mjqjpqmgbljsphdztnvjfqwrcgsmlb");
// findStartOfPacketMarker(
//   ""
// );
function findStartOfMessage(datastreamBuffer) {
    var lastFourteenChars = "";
    console.log("datastreamBuffer", datastreamBuffer);
    for (var i = 0; i < datastreamBuffer.length; i++) {
        // Create 4 letter string and move up the string each iteration
        lastFourteenChars = datastreamBuffer.slice(i, i + 14);
        // count number of occurances
        var letterCount = count(lastFourteenChars);
        // If there are mote than 3 letters counted in
        // length 4 string then they must all be distinct
        if (Object.keys(letterCount).length > 13) {
            console.log(count(lastFourteenChars));
            console.log(i + 14);
            console.log("found it sucker");
            return;
        }
    }
    return -1;
}
findStartOfMessage("");
